  ...
  const sendFile = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/chats/${chatId}/messages`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          content: type.startsWith("image/") ? "Image" : filePreview.file.name,
          type: msgType,
          fileUrl: url,
        }),
      });
      ...
      <div className="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex justify-between items-center shadow-sm">
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          <div className="relative">
            ...
            {/* Dropdown Menu */}
            {showHeaderMenu && (
              <div className="absolute right-0 top-12 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-50 min-w-45 overflow-hidden">
                <button
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  } opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-white dark:bg-gray-800 shadow-sm rounded-full p-1`}
                >
                  <button
                ...
                <div
                  className={`max-w-[85%] sm:max-w-[75%] md:max-w-[70%] rounded-2xl px-3 sm:px-4 py-2 text-sm ${
                    isMe
              ...
              {msg.reactions && msg.reactions.length > 0 && (
                <div
                  className={`flex -mt-2 ${
                    isMe ? "mr-2" : "ml-2"
                  } bg-white dark:bg-gray-800 rounded-full px-1.5 py-0.5 shadow-sm text-xs border border-gray-100 dark:border-gray-700 z-10`}
                >
                  {Array.from(
                    new Set(msg.reactions.map((r: any) => r.type))
                  ).map((reaction: any, i) => (
